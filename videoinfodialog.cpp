...
void VideoInfoDialog::updateinformation(AVFormatContext *update_fmtCtx, const char *filename)
{
    ...
    double frameRate = (double)framerate.num / (double)framerate.den;

    QFileInfo fileInfo(filename);
    QString fileName_ = fileInfo.fileName();

    int totalTime = round(fmtCtx_->duration * av_q2d(AV_TIME_BASE_Q));
    ...
    ui->picFmtLabel->setText("视频像素格式: "+(videoIndex == -1 ? "无视频数据" : QString(charAVPixelFormat[update_fmtCtx->streams[videoIndex]->codecpar->frame_size+1])));
    ui->bitRateLabel->setText("码率: "+QString::number(update_fmtCtx->bit_rate));
    ui->aChannelsLabel->setText("音频声道数: "+(audioIndex == -1 ? "无音频数据" : QString::number(update_fmtCtx->streams[audioIndex]->codecpar->ch_layout.nb_channels)));
    ui->aSampleRateLabel->setText("音频采样率: "+(audioIndex == -1 ? "无音频数据" : QString::number(update_fmtCtx->streams[audioIndex]->codecpar->sample_rate))+"Hz");
